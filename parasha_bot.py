...
# ──────────────────────────────────────────────────────────────────────────────
# ВСТАВЬТЕ ВАШИ КЛЮЧИ (внутри кавычек):
...
async def full_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for i in range(0, len(full), 4000):
        await target.reply_text(full[i:i+4000])
    if len(full) > 4000:
...
def schedule_jobs(app):
    ...
    jq = app.job_queue
    jq.run_daily(lambda ctx: asyncio.create_task(brdc(ctx, True)), time=time(12,20,tzinfo=tz), days=(0,))
    jq.run_daily(lambda ctx: asyncio.create_task(brdc(ctx, False)), time=time(12,20,tzinfo=tz), days=(2,))

async def brdc(context: ContextTypes.DEFAULT_TYPE, is_summary: bool):
    for uid, lang in user_langs.items():
        if is_summary:
            ...
            title = '💡 Чему нас учит глава'
        await context.bot.send_message(chat_id=int(uid), text=f"{title}:\n{text}")
...
def main():
    ...
    app.add_handler(CallbackQueryHandler(lambda u,c: asyncio.create_task(brief_handler(u,c)), pattern=r'^brief$'))
    app.add_handler(CallbackQueryHandler(lambda u,c: asyncio.create_task(full_handler(u,c)),  pattern=r'^full$'))
